...
//配网事件
static EventGroupHandle_t   apcfg_event = NULL;  //定义事件组

//接收到ap配网的ssid和密码
static char current_ssid[32];
static char current_password[64];

#define APCFG_BIT   (BIT0)   //标志位
...
static char* initi_web_page_buffer(void)
{
    ...
    esp_vfs_spiffs_conf_t conf = {
        ...
        .format_if_mount_failed = true    //挂载失败执行格式化--25.11.9测试
        };
    //挂载spiffs
    ...
    char* page = heap_caps_malloc(st.st_size + 1,MALLOC_CAP_SPIRAM);
    if(!page)
    {
        ESP_LOGE(TAG, "内存分配失败，文件大小: %ld", st.st_size);  //补充测试信息 25.11.9
        return NULL;
    }
    ...
    if (fread(page, st.st_size, 1, fp) == 0)
    {
        ...
        ESP_LOGI(TAG, "HTML 文件加载成功，大小: %ld 字节", st.st_size); //补充测试信息 25.11.9
    }
    fclose(fp);
    return page;
}
...
{
    cJSON *root = cJSON_Parse((char *)payload); //root指针为cJSON对象
    if(root)
    {
        ...
        if(scan_js)
        {
            char *scan_value = cJSON_GetStringValue(scan_js); //取字符串内容
        ...
        if(ssid_js && password_js)
        {
            ...
            char *ssid_password = cJSON_GetStringValue(password_js);
            snprintf(current_ssid, sizeof(current_ssid), "%s", ssid_value);
            snprintf(current_password, sizeof(current_password), "%s", ssid_password);
			...
			xEventGroupSetBits(apcfg_event,APCFG_BIT);     //注：当APCFG_BIT置位，即可触发apcfg_event，在ap_wifi_task中执行
        }
...
void ap_wifi_init(p_wifi_state_cb f)
{
    ...
    wifi_manager_init(f);
    apcfg_event = xEventGroupCreate();   //创建事件
